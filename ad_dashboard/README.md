...
| **시간대 분석** | 요일×시간, 채널×시간 히트맵 + 광고비/전환/효율 피크 시간대 — 시간대 입찰·예산 배분 근거 |
...
| 플랫폼 | 시간대 분해 |
|---|---|
| 구글 검색/GDN | ✅ `segments.hour` |
| 메타 | ✅ 광고주 시간대 기준 hourly breakdown |
| 카카오 검색/모먼트 | ✅ `timeUnit=HOUR` |
| 네이버 검색/GFA | ❌ API 미제공 (광고시스템의 대용량 보고서로만 확인 가능) — 수집에서 자동 제외 |

데모 모드에서는 7개 채널 전부 시간대 데이터가 생성됩니다.
